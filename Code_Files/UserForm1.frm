...
Sub buttonClick_Handler(btnID As String, dowhat As doWhat_Enum) 'can be bytes
    If Not Me.tglbtn_Class_OnOff.Value Then
        If btnID <> "" Then
            If Right(btnID, 6) = "Button" Then
                ...
                Me.lbl_WhichLabel_Value.Caption = btnID
            End If
        End If
    End If
End Sub
...
Private Sub cmdbtn_exitButton_Click()
    Me.Hide
End Sub

'____________________Class section___________________________
Private Sub tglbtn_Class_OnOff_Change() 'this is not really required, just to show that class event handling can be turned on/off
    If Me.tglbtn_Class_OnOff.Value Then
        Call instantiateClass 'could be called in the userform initialze event
        Me.tglbtn_Class_OnOff.Caption = "Class ON"
    Else
        Call de_instantiateClass 'not really necessary
        Me.tglbtn_Class_OnOff.Caption = "Class OFF"
    End If
    Me.tglbtn_Class_OnOff.BackColor = IIf(Me.tglbtn_Class_OnOff.Value, vbRed, vbGreen)
    Me.lbl_WhichLabel_Value.Caption = ""
End Sub
